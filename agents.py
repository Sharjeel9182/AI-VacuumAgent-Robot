...
class Environment:
    ...
    def step(self):
        ...
        ag=self.agents[0]
        if not self.is_done(): #runs if the agent is alive
            actions = [] #cretes a list of actions considering that agent is alive
            for agent in self.agents: #looping through all the agents
                if agent.alive: #if agent was alive at a particular index
                    actions.append(agent.program(self.percept(agent))) #the action is to call rule or reflex based
                else:
                    actions.append("")
            for (agent, action) in zip(self.agents, actions):
                self.execute_action(agent, action)
                if (action == 'Suck'):
                    self.dirt = self.dirt - 1
    ...
    def run(self, drt):
        ...
        coun=0
        self.dirt = drt
        print("The dirt number ", drt)
        for step in range(steps):
            if self.is_done1(self.dirt):
                coun=coun+1
...
class VacuumEnvironment(XYEnvironment):
    ...
    def percept(self, agent):
        ...
        count=agent.count
        return status, bump, location1,direction, count
